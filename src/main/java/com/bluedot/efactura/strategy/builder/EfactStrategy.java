...
public class EfactStrategy implements CFEStrategy {
	...
	public EfactStrategy(EFact efactura, TipoDoc tipo) throws EFacturaException {
		...
		if (tipo!=TipoDoc.eFactura && tipo!=TipoDoc.Nota_de_Credito_de_eFactura && tipo!=TipoDoc.Nota_de_Debito_de_eFactura)
	...
	public void setIdDoc() {
		try {
			getEncabezado().setIdDoc(CAEManagerImpl.getInstance().getIdDocFact(tipo));
