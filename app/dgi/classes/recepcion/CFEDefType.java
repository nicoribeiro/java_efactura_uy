//
// This file was generated by the JavaTM Architecture for XML Binding(JAXB) Reference Implementation, v2.2.4-2 
// See <a href="http://java.sun.com/xml/jaxb">http://java.sun.com/xml/jaxb</a> 
// Any modifications to this file will be lost upon recompilation of the source schema. 
// Generated on: 2016.06.01 at 06:36:07 PM UYT 
//


package dgi.classes.recepcion;

import java.util.ArrayList;
import java.util.List;

import javax.xml.bind.annotation.XmlAccessType;
import javax.xml.bind.annotation.XmlAccessorType;
import javax.xml.bind.annotation.XmlAttribute;
import javax.xml.bind.annotation.XmlElement;
import javax.xml.bind.annotation.XmlType;
import javax.xml.datatype.XMLGregorianCalendar;


/**
 * Comprobante Fiscal Electronico
 * 
 * <p>Java class for CFEDefType complex type.
 * 
 * <p>The following schema fragment specifies the expected content contained within this class.
 * 
 * <pre>
 * &lt;complexType name="CFEDefType">
 *   &lt;complexContent>
 *     &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
 *       &lt;sequence>
 *         &lt;choice>
 *           &lt;element name="eTck">
 *             &lt;complexType>
 *               &lt;complexContent>
 *                 &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
 *                   &lt;sequence>
 *                     &lt;element name="TmstFirma" type="{http://cfe.dgi.gub.uy}FechaHoraType"/>
 *                     &lt;element name="Encabezado">
 *                       &lt;complexType>
 *                         &lt;complexContent>
 *                           &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
 *                             &lt;sequence>
 *                               &lt;element name="IdDoc" type="{http://cfe.dgi.gub.uy}IdDoc_Tck"/>
 *                               &lt;element name="Emisor" type="{http://cfe.dgi.gub.uy}Emisor"/>
 *                               &lt;element name="Receptor" type="{http://cfe.dgi.gub.uy}Receptor_Tck" minOccurs="0"/>
 *                               &lt;element name="Totales" type="{http://cfe.dgi.gub.uy}Totales"/>
 *                             &lt;/sequence>
 *                           &lt;/restriction>
 *                         &lt;/complexContent>
 *                       &lt;/complexType>
 *                     &lt;/element>
 *                     &lt;element name="Detalle">
 *                       &lt;complexType>
 *                         &lt;complexContent>
 *                           &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
 *                             &lt;sequence>
 *                               &lt;element name="Item" type="{http://cfe.dgi.gub.uy}Item_Det_Fact" maxOccurs="700"/>
 *                             &lt;/sequence>
 *                           &lt;/restriction>
 *                         &lt;/complexContent>
 *                       &lt;/complexType>
 *                     &lt;/element>
 *                     &lt;element name="SubTotInfo" type="{http://cfe.dgi.gub.uy}SubTotInfo" minOccurs="0"/>
 *                     &lt;element name="DscRcgGlobal" type="{http://cfe.dgi.gub.uy}DscRcgGlobal" minOccurs="0"/>
 *                     &lt;element name="MediosPago" type="{http://cfe.dgi.gub.uy}MediosPago" minOccurs="0"/>
 *                     &lt;element name="Referencia" type="{http://cfe.dgi.gub.uy}ReferenciaTipo" minOccurs="0"/>
 *                     &lt;element name="CAEData" type="{http://cfe.dgi.gub.uy}CAEDataType"/>
 *                     &lt;element name="Compl_Fiscal" type="{http://cfe.dgi.gub.uy}Compl_FiscalType" minOccurs="0"/>
 *                   &lt;/sequence>
 *                 &lt;/restriction>
 *               &lt;/complexContent>
 *             &lt;/complexType>
 *           &lt;/element>
 *           &lt;element name="eFact">
 *             &lt;complexType>
 *               &lt;complexContent>
 *                 &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
 *                   &lt;sequence>
 *                     &lt;element name="TmstFirma" type="{http://cfe.dgi.gub.uy}FechaHoraType"/>
 *                     &lt;element name="Encabezado">
 *                       &lt;complexType>
 *                         &lt;complexContent>
 *                           &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
 *                             &lt;sequence>
 *                               &lt;element name="IdDoc" type="{http://cfe.dgi.gub.uy}IdDoc_Fact"/>
 *                               &lt;element name="Emisor" type="{http://cfe.dgi.gub.uy}Emisor"/>
 *                               &lt;element name="Receptor" type="{http://cfe.dgi.gub.uy}Receptor_Fact"/>
 *                               &lt;element name="Totales" type="{http://cfe.dgi.gub.uy}Totales"/>
 *                             &lt;/sequence>
 *                           &lt;/restriction>
 *                         &lt;/complexContent>
 *                       &lt;/complexType>
 *                     &lt;/element>
 *                     &lt;element name="Detalle">
 *                       &lt;complexType>
 *                         &lt;complexContent>
 *                           &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
 *                             &lt;sequence>
 *                               &lt;element name="Item" type="{http://cfe.dgi.gub.uy}Item_Det_Fact" maxOccurs="200"/>
 *                             &lt;/sequence>
 *                           &lt;/restriction>
 *                         &lt;/complexContent>
 *                       &lt;/complexType>
 *                     &lt;/element>
 *                     &lt;element name="SubTotInfo" type="{http://cfe.dgi.gub.uy}SubTotInfo" minOccurs="0"/>
 *                     &lt;element name="DscRcgGlobal" type="{http://cfe.dgi.gub.uy}DscRcgGlobal" minOccurs="0"/>
 *                     &lt;element name="MediosPago" type="{http://cfe.dgi.gub.uy}MediosPago" minOccurs="0"/>
 *                     &lt;element name="Referencia" type="{http://cfe.dgi.gub.uy}ReferenciaTipo" minOccurs="0"/>
 *                     &lt;element name="CAEData" type="{http://cfe.dgi.gub.uy}CAEDataType"/>
 *                     &lt;element name="Compl_Fiscal" type="{http://cfe.dgi.gub.uy}Compl_FiscalType" minOccurs="0"/>
 *                   &lt;/sequence>
 *                 &lt;/restriction>
 *               &lt;/complexContent>
 *             &lt;/complexType>
 *           &lt;/element>
 *           &lt;element name="eFact_Exp">
 *             &lt;complexType>
 *               &lt;complexContent>
 *                 &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
 *                   &lt;sequence>
 *                     &lt;element name="TmstFirma" type="{http://cfe.dgi.gub.uy}FechaHoraType"/>
 *                     &lt;element name="Encabezado">
 *                       &lt;complexType>
 *                         &lt;complexContent>
 *                           &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
 *                             &lt;sequence>
 *                               &lt;element name="IdDoc" type="{http://cfe.dgi.gub.uy}IdDoc_Fact_Exp"/>
 *                               &lt;element name="Emisor" type="{http://cfe.dgi.gub.uy}Emisor"/>
 *                               &lt;element name="Receptor" type="{http://cfe.dgi.gub.uy}Receptor_Fact_Exp"/>
 *                               &lt;element name="Totales" type="{http://cfe.dgi.gub.uy}Totales_Fact_Exp"/>
 *                             &lt;/sequence>
 *                           &lt;/restriction>
 *                         &lt;/complexContent>
 *                       &lt;/complexType>
 *                     &lt;/element>
 *                     &lt;element name="Detalle">
 *                       &lt;complexType>
 *                         &lt;complexContent>
 *                           &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
 *                             &lt;sequence>
 *                               &lt;element name="Item" type="{http://cfe.dgi.gub.uy}Item_Det_Fact_Exp" maxOccurs="200"/>
 *                             &lt;/sequence>
 *                           &lt;/restriction>
 *                         &lt;/complexContent>
 *                       &lt;/complexType>
 *                     &lt;/element>
 *                     &lt;element name="SubTotInfo" type="{http://cfe.dgi.gub.uy}SubTotInfo" minOccurs="0"/>
 *                     &lt;element name="DscRcgGlobal" type="{http://cfe.dgi.gub.uy}DscRcgGlobal" minOccurs="0"/>
 *                     &lt;element name="MediosPago" type="{http://cfe.dgi.gub.uy}MediosPago" minOccurs="0"/>
 *                     &lt;element name="Referencia" type="{http://cfe.dgi.gub.uy}ReferenciaTipo" minOccurs="0"/>
 *                     &lt;element name="CAEData" type="{http://cfe.dgi.gub.uy}CAEDataType"/>
 *                   &lt;/sequence>
 *                 &lt;/restriction>
 *               &lt;/complexContent>
 *             &lt;/complexType>
 *           &lt;/element>
 *           &lt;element name="eRem">
 *             &lt;complexType>
 *               &lt;complexContent>
 *                 &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
 *                   &lt;sequence>
 *                     &lt;element name="TmstFirma" type="{http://cfe.dgi.gub.uy}FechaHoraType"/>
 *                     &lt;element name="Encabezado">
 *                       &lt;complexType>
 *                         &lt;complexContent>
 *                           &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
 *                             &lt;sequence>
 *                               &lt;element name="IdDoc" type="{http://cfe.dgi.gub.uy}IdDoc_Rem"/>
 *                               &lt;element name="Emisor" type="{http://cfe.dgi.gub.uy}Emisor"/>
 *                               &lt;element name="Receptor" type="{http://cfe.dgi.gub.uy}Receptor_Rem"/>
 *                               &lt;element name="Totales">
 *                                 &lt;complexType>
 *                                   &lt;complexContent>
 *                                     &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
 *                                       &lt;sequence>
 *                                         &lt;element name="CantLinDet">
 *                                           &lt;simpleType>
 *                                             &lt;restriction base="{http://cfe.dgi.gub.uy}LineasDetType">
 *                                               &lt;totalDigits value="3"/>
 *                                               &lt;fractionDigits value="0"/>
 *                                               &lt;maxInclusive value="200"/>
 *                                             &lt;/restriction>
 *                                           &lt;/simpleType>
 *                                         &lt;/element>
 *                                       &lt;/sequence>
 *                                     &lt;/restriction>
 *                                   &lt;/complexContent>
 *                                 &lt;/complexType>
 *                               &lt;/element>
 *                             &lt;/sequence>
 *                           &lt;/restriction>
 *                         &lt;/complexContent>
 *                       &lt;/complexType>
 *                     &lt;/element>
 *                     &lt;element name="Detalle">
 *                       &lt;complexType>
 *                         &lt;complexContent>
 *                           &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
 *                             &lt;sequence>
 *                               &lt;element name="Item" type="{http://cfe.dgi.gub.uy}Item_Rem" maxOccurs="200"/>
 *                             &lt;/sequence>
 *                           &lt;/restriction>
 *                         &lt;/complexContent>
 *                       &lt;/complexType>
 *                     &lt;/element>
 *                     &lt;element name="SubTotInfo" type="{http://cfe.dgi.gub.uy}SubTotInfo" minOccurs="0"/>
 *                     &lt;element name="Referencia" type="{http://cfe.dgi.gub.uy}ReferenciaTipo" minOccurs="0"/>
 *                     &lt;element name="CAEData" type="{http://cfe.dgi.gub.uy}CAEDataType"/>
 *                   &lt;/sequence>
 *                 &lt;/restriction>
 *               &lt;/complexContent>
 *             &lt;/complexType>
 *           &lt;/element>
 *           &lt;element name="eRem_Exp">
 *             &lt;complexType>
 *               &lt;complexContent>
 *                 &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
 *                   &lt;sequence>
 *                     &lt;element name="TmstFirma" type="{http://cfe.dgi.gub.uy}FechaHoraType"/>
 *                     &lt;element name="Encabezado">
 *                       &lt;complexType>
 *                         &lt;complexContent>
 *                           &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
 *                             &lt;sequence>
 *                               &lt;element name="IdDoc" type="{http://cfe.dgi.gub.uy}IdDoc_Rem_Exp"/>
 *                               &lt;element name="Emisor" type="{http://cfe.dgi.gub.uy}Emisor"/>
 *                               &lt;element name="Receptor" type="{http://cfe.dgi.gub.uy}Receptor_Rem_Exp"/>
 *                               &lt;element name="Totales" type="{http://cfe.dgi.gub.uy}Totales_Rem_Exp"/>
 *                             &lt;/sequence>
 *                           &lt;/restriction>
 *                         &lt;/complexContent>
 *                       &lt;/complexType>
 *                     &lt;/element>
 *                     &lt;element name="Detalle">
 *                       &lt;complexType>
 *                         &lt;complexContent>
 *                           &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
 *                             &lt;sequence>
 *                               &lt;element name="Item" type="{http://cfe.dgi.gub.uy}Item_Rem_Exp" maxOccurs="200"/>
 *                             &lt;/sequence>
 *                           &lt;/restriction>
 *                         &lt;/complexContent>
 *                       &lt;/complexType>
 *                     &lt;/element>
 *                     &lt;element name="SubTotInfo" type="{http://cfe.dgi.gub.uy}SubTotInfo" minOccurs="0"/>
 *                     &lt;element name="Referencia" type="{http://cfe.dgi.gub.uy}ReferenciaTipo" minOccurs="0"/>
 *                     &lt;element name="CAEData" type="{http://cfe.dgi.gub.uy}CAEDataType"/>
 *                   &lt;/sequence>
 *                 &lt;/restriction>
 *               &lt;/complexContent>
 *             &lt;/complexType>
 *           &lt;/element>
 *           &lt;element name="eResg">
 *             &lt;complexType>
 *               &lt;complexContent>
 *                 &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
 *                   &lt;sequence>
 *                     &lt;element name="TmstFirma" type="{http://cfe.dgi.gub.uy}FechaHoraType"/>
 *                     &lt;element name="Encabezado">
 *                       &lt;complexType>
 *                         &lt;complexContent>
 *                           &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
 *                             &lt;sequence>
 *                               &lt;element name="IdDoc" type="{http://cfe.dgi.gub.uy}IdDoc_Resg"/>
 *                               &lt;element name="Emisor" type="{http://cfe.dgi.gub.uy}Emisor"/>
 *                               &lt;element name="Receptor" type="{http://cfe.dgi.gub.uy}Receptor_Resg"/>
 *                               &lt;element name="Totales" type="{http://cfe.dgi.gub.uy}Totales_Resg"/>
 *                             &lt;/sequence>
 *                           &lt;/restriction>
 *                         &lt;/complexContent>
 *                       &lt;/complexType>
 *                     &lt;/element>
 *                     &lt;element name="Detalle">
 *                       &lt;complexType>
 *                         &lt;complexContent>
 *                           &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
 *                             &lt;sequence>
 *                               &lt;element name="Item" type="{http://cfe.dgi.gub.uy}Item_Resg" maxOccurs="200"/>
 *                             &lt;/sequence>
 *                           &lt;/restriction>
 *                         &lt;/complexContent>
 *                       &lt;/complexType>
 *                     &lt;/element>
 *                     &lt;element name="SubTotInfo" type="{http://cfe.dgi.gub.uy}SubTotInfo" minOccurs="0"/>
 *                     &lt;element name="Referencia" type="{http://cfe.dgi.gub.uy}ReferenciaTipo" minOccurs="0"/>
 *                     &lt;element name="CAEData" type="{http://cfe.dgi.gub.uy}CAEDataType"/>
 *                   &lt;/sequence>
 *                 &lt;/restriction>
 *               &lt;/complexContent>
 *             &lt;/complexType>
 *           &lt;/element>
 *         &lt;/choice>
 *         &lt;element ref="{http://www.w3.org/2000/09/xmldsig#}Signature"/>
 *       &lt;/sequence>
 *       &lt;attribute name="version" use="required" fixed="1.0">
 *         &lt;simpleType>
 *           &lt;restriction base="{http://www.w3.org/2001/XMLSchema}string">
 *             &lt;maxLength value="3"/>
 *           &lt;/restriction>
 *         &lt;/simpleType>
 *       &lt;/attribute>
 *     &lt;/restriction>
 *   &lt;/complexContent>
 * &lt;/complexType>
 * </pre>
 * 
 * 
 */
@XmlAccessorType(XmlAccessType.FIELD)
@XmlType(name = "CFEDefType", namespace = "http://cfe.dgi.gub.uy", propOrder = {
    "eResg",
    "eRemExp",
    "eRem",
    "eFactExp",
    "eFact",
    "eTck",
    "signature"
})
public class CFEDefType {

    protected CFEDefType.EResg eResg;
    @XmlElement(name = "eRem_Exp")
    protected CFEDefType.ERemExp eRemExp;
    protected CFEDefType.ERem eRem;
    @XmlElement(name = "eFact_Exp")
    protected CFEDefType.EFactExp eFactExp;
    protected CFEDefType.EFact eFact;
    protected CFEDefType.ETck eTck;
    @XmlElement(name = "Signature", namespace = "http://www.w3.org/2000/09/xmldsig#", required = true)
    protected Signature signature;
    @XmlAttribute(name = "version", required = true)
    protected String version;

    /**
     * Gets the value of the eResg property.
     * 
     * @return
     *     possible object is
     *     {@link CFEDefType.EResg }
     *     
     */
    public CFEDefType.EResg getEResg() {
        return eResg;
    }

    /**
     * Sets the value of the eResg property.
     * 
     * @param value
     *     allowed object is
     *     {@link CFEDefType.EResg }
     *     
     */
    public void setEResg(CFEDefType.EResg value) {
        this.eResg = value;
    }

    /**
     * Gets the value of the eRemExp property.
     * 
     * @return
     *     possible object is
     *     {@link CFEDefType.ERemExp }
     *     
     */
    public CFEDefType.ERemExp getERemExp() {
        return eRemExp;
    }

    /**
     * Sets the value of the eRemExp property.
     * 
     * @param value
     *     allowed object is
     *     {@link CFEDefType.ERemExp }
     *     
     */
    public void setERemExp(CFEDefType.ERemExp value) {
        this.eRemExp = value;
    }

    /**
     * Gets the value of the eRem property.
     * 
     * @return
     *     possible object is
     *     {@link CFEDefType.ERem }
     *     
     */
    public CFEDefType.ERem getERem() {
        return eRem;
    }

    /**
     * Sets the value of the eRem property.
     * 
     * @param value
     *     allowed object is
     *     {@link CFEDefType.ERem }
     *     
     */
    public void setERem(CFEDefType.ERem value) {
        this.eRem = value;
    }

    /**
     * Gets the value of the eFactExp property.
     * 
     * @return
     *     possible object is
     *     {@link CFEDefType.EFactExp }
     *     
     */
    public CFEDefType.EFactExp getEFactExp() {
        return eFactExp;
    }

    /**
     * Sets the value of the eFactExp property.
     * 
     * @param value
     *     allowed object is
     *     {@link CFEDefType.EFactExp }
     *     
     */
    public void setEFactExp(CFEDefType.EFactExp value) {
        this.eFactExp = value;
    }

    /**
     * Gets the value of the eFact property.
     * 
     * @return
     *     possible object is
     *     {@link CFEDefType.EFact }
     *     
     */
    public CFEDefType.EFact getEFact() {
        return eFact;
    }

    /**
     * Sets the value of the eFact property.
     * 
     * @param value
     *     allowed object is
     *     {@link CFEDefType.EFact }
     *     
     */
    public void setEFact(CFEDefType.EFact value) {
        this.eFact = value;
    }

    /**
     * Gets the value of the eTck property.
     * 
     * @return
     *     possible object is
     *     {@link CFEDefType.ETck }
     *     
     */
    public CFEDefType.ETck getETck() {
        return eTck;
    }

    /**
     * Sets the value of the eTck property.
     * 
     * @param value
     *     allowed object is
     *     {@link CFEDefType.ETck }
     *     
     */
    public void setETck(CFEDefType.ETck value) {
        this.eTck = value;
    }

    /**
     * Gets the value of the signature property.
     * 
     * @return
     *     possible object is
     *     {@link Signature }
     *     
     */
    public Signature getSignature() {
        return signature;
    }

    /**
     * Sets the value of the signature property.
     * 
     * @param value
     *     allowed object is
     *     {@link Signature }
     *     
     */
    public void setSignature(Signature value) {
        this.signature = value;
    }

    /**
     * Gets the value of the version property.
     * 
     * @return
     *     possible object is
     *     {@link String }
     *     
     */
    public String getVersion() {
        if (version == null) {
            return "1.0";
        } else {
            return version;
        }
    }

    /**
     * Sets the value of the version property.
     * 
     * @param value
     *     allowed object is
     *     {@link String }
     *     
     */
    public void setVersion(String value) {
        this.version = value;
    }


    /**
     * <p>Java class for anonymous complex type.
     * 
     * <p>The following schema fragment specifies the expected content contained within this class.
     * 
     * <pre>
     * &lt;complexType>
     *   &lt;complexContent>
     *     &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
     *       &lt;sequence>
     *         &lt;element name="TmstFirma" type="{http://cfe.dgi.gub.uy}FechaHoraType"/>
     *         &lt;element name="Encabezado">
     *           &lt;complexType>
     *             &lt;complexContent>
     *               &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
     *                 &lt;sequence>
     *                   &lt;element name="IdDoc" type="{http://cfe.dgi.gub.uy}IdDoc_Fact"/>
     *                   &lt;element name="Emisor" type="{http://cfe.dgi.gub.uy}Emisor"/>
     *                   &lt;element name="Receptor" type="{http://cfe.dgi.gub.uy}Receptor_Fact"/>
     *                   &lt;element name="Totales" type="{http://cfe.dgi.gub.uy}Totales"/>
     *                 &lt;/sequence>
     *               &lt;/restriction>
     *             &lt;/complexContent>
     *           &lt;/complexType>
     *         &lt;/element>
     *         &lt;element name="Detalle">
     *           &lt;complexType>
     *             &lt;complexContent>
     *               &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
     *                 &lt;sequence>
     *                   &lt;element name="Item" type="{http://cfe.dgi.gub.uy}Item_Det_Fact" maxOccurs="200"/>
     *                 &lt;/sequence>
     *               &lt;/restriction>
     *             &lt;/complexContent>
     *           &lt;/complexType>
     *         &lt;/element>
     *         &lt;element name="SubTotInfo" type="{http://cfe.dgi.gub.uy}SubTotInfo" minOccurs="0"/>
     *         &lt;element name="DscRcgGlobal" type="{http://cfe.dgi.gub.uy}DscRcgGlobal" minOccurs="0"/>
     *         &lt;element name="MediosPago" type="{http://cfe.dgi.gub.uy}MediosPago" minOccurs="0"/>
     *         &lt;element name="Referencia" type="{http://cfe.dgi.gub.uy}ReferenciaTipo" minOccurs="0"/>
     *         &lt;element name="CAEData" type="{http://cfe.dgi.gub.uy}CAEDataType"/>
     *         &lt;element name="Compl_Fiscal" type="{http://cfe.dgi.gub.uy}Compl_FiscalType" minOccurs="0"/>
     *       &lt;/sequence>
     *     &lt;/restriction>
     *   &lt;/complexContent>
     * &lt;/complexType>
     * </pre>
     * 
     * 
     */
    @XmlAccessorType(XmlAccessType.FIELD)
    @XmlType(name = "", propOrder = {
        "tmstFirma",
        "encabezado",
        "detalle",
        "subTotInfo",
        "dscRcgGlobal",
        "mediosPago",
        "referencia",
        "caeData",
        "complFiscal"
    })
    public static class EFact {

        @XmlElement(name = "TmstFirma", namespace = "http://cfe.dgi.gub.uy", required = true)
        protected XMLGregorianCalendar tmstFirma;
        @XmlElement(name = "Encabezado", namespace = "http://cfe.dgi.gub.uy", required = true)
        protected CFEDefType.EFact.Encabezado encabezado;
        @XmlElement(name = "Detalle", namespace = "http://cfe.dgi.gub.uy", required = true)
        protected CFEDefType.EFact.Detalle detalle;
        @XmlElement(name = "SubTotInfo", namespace = "http://cfe.dgi.gub.uy")
        protected SubTotInfo subTotInfo;
        @XmlElement(name = "DscRcgGlobal", namespace = "http://cfe.dgi.gub.uy")
        protected DscRcgGlobal dscRcgGlobal;
        @XmlElement(name = "MediosPago", namespace = "http://cfe.dgi.gub.uy")
        protected MediosPago mediosPago;
        @XmlElement(name = "Referencia", namespace = "http://cfe.dgi.gub.uy")
        protected ReferenciaTipo referencia;
        @XmlElement(name = "CAEData", namespace = "http://cfe.dgi.gub.uy", required = true)
        protected CAEDataType caeData;
        @XmlElement(name = "Compl_Fiscal", namespace = "http://cfe.dgi.gub.uy")
        protected ComplFiscalType complFiscal;

        /**
         * Gets the value of the tmstFirma property.
         * 
         * @return
         *     possible object is
         *     {@link XMLGregorianCalendar }
         *     
         */
        public XMLGregorianCalendar getTmstFirma() {
            return tmstFirma;
        }

        /**
         * Sets the value of the tmstFirma property.
         * 
         * @param value
         *     allowed object is
         *     {@link XMLGregorianCalendar }
         *     
         */
        public void setTmstFirma(XMLGregorianCalendar value) {
            this.tmstFirma = value;
        }

        /**
         * Gets the value of the encabezado property.
         * 
         * @return
         *     possible object is
         *     {@link CFEDefType.EFact.Encabezado }
         *     
         */
        public CFEDefType.EFact.Encabezado getEncabezado() {
            return encabezado;
        }

        /**
         * Sets the value of the encabezado property.
         * 
         * @param value
         *     allowed object is
         *     {@link CFEDefType.EFact.Encabezado }
         *     
         */
        public void setEncabezado(CFEDefType.EFact.Encabezado value) {
            this.encabezado = value;
        }

        /**
         * Gets the value of the detalle property.
         * 
         * @return
         *     possible object is
         *     {@link CFEDefType.EFact.Detalle }
         *     
         */
        public CFEDefType.EFact.Detalle getDetalle() {
            return detalle;
        }

        /**
         * Sets the value of the detalle property.
         * 
         * @param value
         *     allowed object is
         *     {@link CFEDefType.EFact.Detalle }
         *     
         */
        public void setDetalle(CFEDefType.EFact.Detalle value) {
            this.detalle = value;
        }

        /**
         * Gets the value of the subTotInfo property.
         * 
         * @return
         *     possible object is
         *     {@link SubTotInfo }
         *     
         */
        public SubTotInfo getSubTotInfo() {
            return subTotInfo;
        }

        /**
         * Sets the value of the subTotInfo property.
         * 
         * @param value
         *     allowed object is
         *     {@link SubTotInfo }
         *     
         */
        public void setSubTotInfo(SubTotInfo value) {
            this.subTotInfo = value;
        }

        /**
         * Gets the value of the dscRcgGlobal property.
         * 
         * @return
         *     possible object is
         *     {@link DscRcgGlobal }
         *     
         */
        public DscRcgGlobal getDscRcgGlobal() {
            return dscRcgGlobal;
        }

        /**
         * Sets the value of the dscRcgGlobal property.
         * 
         * @param value
         *     allowed object is
         *     {@link DscRcgGlobal }
         *     
         */
        public void setDscRcgGlobal(DscRcgGlobal value) {
            this.dscRcgGlobal = value;
        }

        /**
         * Gets the value of the mediosPago property.
         * 
         * @return
         *     possible object is
         *     {@link MediosPago }
         *     
         */
        public MediosPago getMediosPago() {
            return mediosPago;
        }

        /**
         * Sets the value of the mediosPago property.
         * 
         * @param value
         *     allowed object is
         *     {@link MediosPago }
         *     
         */
        public void setMediosPago(MediosPago value) {
            this.mediosPago = value;
        }

        /**
         * Gets the value of the referencia property.
         * 
         * @return
         *     possible object is
         *     {@link ReferenciaTipo }
         *     
         */
        public ReferenciaTipo getReferencia() {
            return referencia;
        }

        /**
         * Sets the value of the referencia property.
         * 
         * @param value
         *     allowed object is
         *     {@link ReferenciaTipo }
         *     
         */
        public void setReferencia(ReferenciaTipo value) {
            this.referencia = value;
        }

        /**
         * Gets the value of the caeData property.
         * 
         * @return
         *     possible object is
         *     {@link CAEDataType }
         *     
         */
        public CAEDataType getCAEData() {
            return caeData;
        }

        /**
         * Sets the value of the caeData property.
         * 
         * @param value
         *     allowed object is
         *     {@link CAEDataType }
         *     
         */
        public void setCAEData(CAEDataType value) {
            this.caeData = value;
        }

        /**
         * Gets the value of the complFiscal property.
         * 
         * @return
         *     possible object is
         *     {@link ComplFiscalType }
         *     
         */
        public ComplFiscalType getComplFiscal() {
            return complFiscal;
        }

        /**
         * Sets the value of the complFiscal property.
         * 
         * @param value
         *     allowed object is
         *     {@link ComplFiscalType }
         *     
         */
        public void setComplFiscal(ComplFiscalType value) {
            this.complFiscal = value;
        }


        /**
         * <p>Java class for anonymous complex type.
         * 
         * <p>The following schema fragment specifies the expected content contained within this class.
         * 
         * <pre>
         * &lt;complexType>
         *   &lt;complexContent>
         *     &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
         *       &lt;sequence>
         *         &lt;element name="Item" type="{http://cfe.dgi.gub.uy}Item_Det_Fact" maxOccurs="200"/>
         *       &lt;/sequence>
         *     &lt;/restriction>
         *   &lt;/complexContent>
         * &lt;/complexType>
         * </pre>
         * 
         * 
         */
        @XmlAccessorType(XmlAccessType.FIELD)
        @XmlType(name = "", propOrder = {
            "items"
        })
        public static class Detalle {

            @XmlElement(name = "Item", namespace = "http://cfe.dgi.gub.uy", required = true)
            protected List<ItemDetFact> items;

            /**
             * Gets the value of the items property.
             * 
             * <p>
             * This accessor method returns a reference to the live list,
             * not a snapshot. Therefore any modification you make to the
             * returned list will be present inside the JAXB object.
             * This is why there is not a <CODE>set</CODE> method for the items property.
             * 
             * <p>
             * For example, to add a new item, do as follows:
             * <pre>
             *    getItems().add(newItem);
             * </pre>
             * 
             * 
             * <p>
             * Objects of the following type(s) are allowed in the list
             * {@link ItemDetFact }
             * 
             * 
             */
            public List<ItemDetFact> getItems() {
                if (items == null) {
                    items = new ArrayList<ItemDetFact>();
                }
                return this.items;
            }

        }


        /**
         * <p>Java class for anonymous complex type.
         * 
         * <p>The following schema fragment specifies the expected content contained within this class.
         * 
         * <pre>
         * &lt;complexType>
         *   &lt;complexContent>
         *     &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
         *       &lt;sequence>
         *         &lt;element name="IdDoc" type="{http://cfe.dgi.gub.uy}IdDoc_Fact"/>
         *         &lt;element name="Emisor" type="{http://cfe.dgi.gub.uy}Emisor"/>
         *         &lt;element name="Receptor" type="{http://cfe.dgi.gub.uy}Receptor_Fact"/>
         *         &lt;element name="Totales" type="{http://cfe.dgi.gub.uy}Totales"/>
         *       &lt;/sequence>
         *     &lt;/restriction>
         *   &lt;/complexContent>
         * &lt;/complexType>
         * </pre>
         * 
         * 
         */
        @XmlAccessorType(XmlAccessType.FIELD)
        @XmlType(name = "", propOrder = {
            "idDoc",
            "emisor",
            "receptor",
            "totales"
        })
        public static class Encabezado {

            @XmlElement(name = "IdDoc", namespace = "http://cfe.dgi.gub.uy", required = true)
            protected IdDocFact idDoc;
            @XmlElement(name = "Emisor", namespace = "http://cfe.dgi.gub.uy", required = true)
            protected Emisor emisor;
            @XmlElement(name = "Receptor", namespace = "http://cfe.dgi.gub.uy", required = true)
            protected ReceptorFact receptor;
            @XmlElement(name = "Totales", namespace = "http://cfe.dgi.gub.uy", required = true)
            protected dgi.classes.recepcion.Totales totales;

            /**
             * Gets the value of the idDoc property.
             * 
             * @return
             *     possible object is
             *     {@link IdDocFact }
             *     
             */
            public IdDocFact getIdDoc() {
                return idDoc;
            }

            /**
             * Sets the value of the idDoc property.
             * 
             * @param value
             *     allowed object is
             *     {@link IdDocFact }
             *     
             */
            public void setIdDoc(IdDocFact value) {
                this.idDoc = value;
            }

            /**
             * Gets the value of the emisor property.
             * 
             * @return
             *     possible object is
             *     {@link Emisor }
             *     
             */
            public Emisor getEmisor() {
                return emisor;
            }

            /**
             * Sets the value of the emisor property.
             * 
             * @param value
             *     allowed object is
             *     {@link Emisor }
             *     
             */
            public void setEmisor(Emisor value) {
                this.emisor = value;
            }

            /**
             * Gets the value of the receptor property.
             * 
             * @return
             *     possible object is
             *     {@link ReceptorFact }
             *     
             */
            public ReceptorFact getReceptor() {
                return receptor;
            }

            /**
             * Sets the value of the receptor property.
             * 
             * @param value
             *     allowed object is
             *     {@link ReceptorFact }
             *     
             */
            public void setReceptor(ReceptorFact value) {
                this.receptor = value;
            }

            /**
             * Gets the value of the totales property.
             * 
             * @return
             *     possible object is
             *     {@link dgi.classes.recepcion.Totales }
             *     
             */
            public dgi.classes.recepcion.Totales getTotales() {
                return totales;
            }

            /**
             * Sets the value of the totales property.
             * 
             * @param value
             *     allowed object is
             *     {@link dgi.classes.recepcion.Totales }
             *     
             */
            public void setTotales(dgi.classes.recepcion.Totales value) {
                this.totales = value;
            }

        }

    }


    /**
     * <p>Java class for anonymous complex type.
     * 
     * <p>The following schema fragment specifies the expected content contained within this class.
     * 
     * <pre>
     * &lt;complexType>
     *   &lt;complexContent>
     *     &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
     *       &lt;sequence>
     *         &lt;element name="TmstFirma" type="{http://cfe.dgi.gub.uy}FechaHoraType"/>
     *         &lt;element name="Encabezado">
     *           &lt;complexType>
     *             &lt;complexContent>
     *               &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
     *                 &lt;sequence>
     *                   &lt;element name="IdDoc" type="{http://cfe.dgi.gub.uy}IdDoc_Fact_Exp"/>
     *                   &lt;element name="Emisor" type="{http://cfe.dgi.gub.uy}Emisor"/>
     *                   &lt;element name="Receptor" type="{http://cfe.dgi.gub.uy}Receptor_Fact_Exp"/>
     *                   &lt;element name="Totales" type="{http://cfe.dgi.gub.uy}Totales_Fact_Exp"/>
     *                 &lt;/sequence>
     *               &lt;/restriction>
     *             &lt;/complexContent>
     *           &lt;/complexType>
     *         &lt;/element>
     *         &lt;element name="Detalle">
     *           &lt;complexType>
     *             &lt;complexContent>
     *               &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
     *                 &lt;sequence>
     *                   &lt;element name="Item" type="{http://cfe.dgi.gub.uy}Item_Det_Fact_Exp" maxOccurs="200"/>
     *                 &lt;/sequence>
     *               &lt;/restriction>
     *             &lt;/complexContent>
     *           &lt;/complexType>
     *         &lt;/element>
     *         &lt;element name="SubTotInfo" type="{http://cfe.dgi.gub.uy}SubTotInfo" minOccurs="0"/>
     *         &lt;element name="DscRcgGlobal" type="{http://cfe.dgi.gub.uy}DscRcgGlobal" minOccurs="0"/>
     *         &lt;element name="MediosPago" type="{http://cfe.dgi.gub.uy}MediosPago" minOccurs="0"/>
     *         &lt;element name="Referencia" type="{http://cfe.dgi.gub.uy}ReferenciaTipo" minOccurs="0"/>
     *         &lt;element name="CAEData" type="{http://cfe.dgi.gub.uy}CAEDataType"/>
     *       &lt;/sequence>
     *     &lt;/restriction>
     *   &lt;/complexContent>
     * &lt;/complexType>
     * </pre>
     * 
     * 
     */
    @XmlAccessorType(XmlAccessType.FIELD)
    @XmlType(name = "", propOrder = {
        "tmstFirma",
        "encabezado",
        "detalle",
        "subTotInfo",
        "dscRcgGlobal",
        "mediosPago",
        "referencia",
        "caeData"
    })
    public static class EFactExp {

        @XmlElement(name = "TmstFirma", namespace = "http://cfe.dgi.gub.uy", required = true)
        protected XMLGregorianCalendar tmstFirma;
        @XmlElement(name = "Encabezado", namespace = "http://cfe.dgi.gub.uy", required = true)
        protected CFEDefType.EFactExp.Encabezado encabezado;
        @XmlElement(name = "Detalle", namespace = "http://cfe.dgi.gub.uy", required = true)
        protected CFEDefType.EFactExp.Detalle detalle;
        @XmlElement(name = "SubTotInfo", namespace = "http://cfe.dgi.gub.uy")
        protected SubTotInfo subTotInfo;
        @XmlElement(name = "DscRcgGlobal", namespace = "http://cfe.dgi.gub.uy")
        protected DscRcgGlobal dscRcgGlobal;
        @XmlElement(name = "MediosPago", namespace = "http://cfe.dgi.gub.uy")
        protected MediosPago mediosPago;
        @XmlElement(name = "Referencia", namespace = "http://cfe.dgi.gub.uy")
        protected ReferenciaTipo referencia;
        @XmlElement(name = "CAEData", namespace = "http://cfe.dgi.gub.uy", required = true)
        protected CAEDataType caeData;

        /**
         * Gets the value of the tmstFirma property.
         * 
         * @return
         *     possible object is
         *     {@link XMLGregorianCalendar }
         *     
         */
        public XMLGregorianCalendar getTmstFirma() {
            return tmstFirma;
        }

        /**
         * Sets the value of the tmstFirma property.
         * 
         * @param value
         *     allowed object is
         *     {@link XMLGregorianCalendar }
         *     
         */
        public void setTmstFirma(XMLGregorianCalendar value) {
            this.tmstFirma = value;
        }

        /**
         * Gets the value of the encabezado property.
         * 
         * @return
         *     possible object is
         *     {@link CFEDefType.EFactExp.Encabezado }
         *     
         */
        public CFEDefType.EFactExp.Encabezado getEncabezado() {
            return encabezado;
        }

        /**
         * Sets the value of the encabezado property.
         * 
         * @param value
         *     allowed object is
         *     {@link CFEDefType.EFactExp.Encabezado }
         *     
         */
        public void setEncabezado(CFEDefType.EFactExp.Encabezado value) {
            this.encabezado = value;
        }

        /**
         * Gets the value of the detalle property.
         * 
         * @return
         *     possible object is
         *     {@link CFEDefType.EFactExp.Detalle }
         *     
         */
        public CFEDefType.EFactExp.Detalle getDetalle() {
            return detalle;
        }

        /**
         * Sets the value of the detalle property.
         * 
         * @param value
         *     allowed object is
         *     {@link CFEDefType.EFactExp.Detalle }
         *     
         */
        public void setDetalle(CFEDefType.EFactExp.Detalle value) {
            this.detalle = value;
        }

        /**
         * Gets the value of the subTotInfo property.
         * 
         * @return
         *     possible object is
         *     {@link SubTotInfo }
         *     
         */
        public SubTotInfo getSubTotInfo() {
            return subTotInfo;
        }

        /**
         * Sets the value of the subTotInfo property.
         * 
         * @param value
         *     allowed object is
         *     {@link SubTotInfo }
         *     
         */
        public void setSubTotInfo(SubTotInfo value) {
            this.subTotInfo = value;
        }

        /**
         * Gets the value of the dscRcgGlobal property.
         * 
         * @return
         *     possible object is
         *     {@link DscRcgGlobal }
         *     
         */
        public DscRcgGlobal getDscRcgGlobal() {
            return dscRcgGlobal;
        }

        /**
         * Sets the value of the dscRcgGlobal property.
         * 
         * @param value
         *     allowed object is
         *     {@link DscRcgGlobal }
         *     
         */
        public void setDscRcgGlobal(DscRcgGlobal value) {
            this.dscRcgGlobal = value;
        }

        /**
         * Gets the value of the mediosPago property.
         * 
         * @return
         *     possible object is
         *     {@link MediosPago }
         *     
         */
        public MediosPago getMediosPago() {
            return mediosPago;
        }

        /**
         * Sets the value of the mediosPago property.
         * 
         * @param value
         *     allowed object is
         *     {@link MediosPago }
         *     
         */
        public void setMediosPago(MediosPago value) {
            this.mediosPago = value;
        }

        /**
         * Gets the value of the referencia property.
         * 
         * @return
         *     possible object is
         *     {@link ReferenciaTipo }
         *     
         */
        public ReferenciaTipo getReferencia() {
            return referencia;
        }

        /**
         * Sets the value of the referencia property.
         * 
         * @param value
         *     allowed object is
         *     {@link ReferenciaTipo }
         *     
         */
        public void setReferencia(ReferenciaTipo value) {
            this.referencia = value;
        }

        /**
         * Gets the value of the caeData property.
         * 
         * @return
         *     possible object is
         *     {@link CAEDataType }
         *     
         */
        public CAEDataType getCAEData() {
            return caeData;
        }

        /**
         * Sets the value of the caeData property.
         * 
         * @param value
         *     allowed object is
         *     {@link CAEDataType }
         *     
         */
        public void setCAEData(CAEDataType value) {
            this.caeData = value;
        }


        /**
         * <p>Java class for anonymous complex type.
         * 
         * <p>The following schema fragment specifies the expected content contained within this class.
         * 
         * <pre>
         * &lt;complexType>
         *   &lt;complexContent>
         *     &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
         *       &lt;sequence>
         *         &lt;element name="Item" type="{http://cfe.dgi.gub.uy}Item_Det_Fact_Exp" maxOccurs="200"/>
         *       &lt;/sequence>
         *     &lt;/restriction>
         *   &lt;/complexContent>
         * &lt;/complexType>
         * </pre>
         * 
         * 
         */
        @XmlAccessorType(XmlAccessType.FIELD)
        @XmlType(name = "", propOrder = {
            "items"
        })
        public static class Detalle {

            @XmlElement(name = "Item", namespace = "http://cfe.dgi.gub.uy", required = true)
            protected List<ItemDetFactExp> items;

            /**
             * Gets the value of the items property.
             * 
             * <p>
             * This accessor method returns a reference to the live list,
             * not a snapshot. Therefore any modification you make to the
             * returned list will be present inside the JAXB object.
             * This is why there is not a <CODE>set</CODE> method for the items property.
             * 
             * <p>
             * For example, to add a new item, do as follows:
             * <pre>
             *    getItems().add(newItem);
             * </pre>
             * 
             * 
             * <p>
             * Objects of the following type(s) are allowed in the list
             * {@link ItemDetFactExp }
             * 
             * 
             */
            public List<ItemDetFactExp> getItems() {
                if (items == null) {
                    items = new ArrayList<ItemDetFactExp>();
                }
                return this.items;
            }

        }


        /**
         * <p>Java class for anonymous complex type.
         * 
         * <p>The following schema fragment specifies the expected content contained within this class.
         * 
         * <pre>
         * &lt;complexType>
         *   &lt;complexContent>
         *     &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
         *       &lt;sequence>
         *         &lt;element name="IdDoc" type="{http://cfe.dgi.gub.uy}IdDoc_Fact_Exp"/>
         *         &lt;element name="Emisor" type="{http://cfe.dgi.gub.uy}Emisor"/>
         *         &lt;element name="Receptor" type="{http://cfe.dgi.gub.uy}Receptor_Fact_Exp"/>
         *         &lt;element name="Totales" type="{http://cfe.dgi.gub.uy}Totales_Fact_Exp"/>
         *       &lt;/sequence>
         *     &lt;/restriction>
         *   &lt;/complexContent>
         * &lt;/complexType>
         * </pre>
         * 
         * 
         */
        @XmlAccessorType(XmlAccessType.FIELD)
        @XmlType(name = "", propOrder = {
            "idDoc",
            "emisor",
            "receptor",
            "totales"
        })
        public static class Encabezado {

            @XmlElement(name = "IdDoc", namespace = "http://cfe.dgi.gub.uy", required = true)
            protected IdDocFactExp idDoc;
            @XmlElement(name = "Emisor", namespace = "http://cfe.dgi.gub.uy", required = true)
            protected Emisor emisor;
            @XmlElement(name = "Receptor", namespace = "http://cfe.dgi.gub.uy", required = true)
            protected ReceptorFactExp receptor;
            @XmlElement(name = "Totales", namespace = "http://cfe.dgi.gub.uy", required = true)
            protected TotalesFactExp totales;

            /**
             * Gets the value of the idDoc property.
             * 
             * @return
             *     possible object is
             *     {@link IdDocFactExp }
             *     
             */
            public IdDocFactExp getIdDoc() {
                return idDoc;
            }

            /**
             * Sets the value of the idDoc property.
             * 
             * @param value
             *     allowed object is
             *     {@link IdDocFactExp }
             *     
             */
            public void setIdDoc(IdDocFactExp value) {
                this.idDoc = value;
            }

            /**
             * Gets the value of the emisor property.
             * 
             * @return
             *     possible object is
             *     {@link Emisor }
             *     
             */
            public Emisor getEmisor() {
                return emisor;
            }

            /**
             * Sets the value of the emisor property.
             * 
             * @param value
             *     allowed object is
             *     {@link Emisor }
             *     
             */
            public void setEmisor(Emisor value) {
                this.emisor = value;
            }

            /**
             * Gets the value of the receptor property.
             * 
             * @return
             *     possible object is
             *     {@link ReceptorFactExp }
             *     
             */
            public ReceptorFactExp getReceptor() {
                return receptor;
            }

            /**
             * Sets the value of the receptor property.
             * 
             * @param value
             *     allowed object is
             *     {@link ReceptorFactExp }
             *     
             */
            public void setReceptor(ReceptorFactExp value) {
                this.receptor = value;
            }

            /**
             * Gets the value of the totales property.
             * 
             * @return
             *     possible object is
             *     {@link TotalesFactExp }
             *     
             */
            public TotalesFactExp getTotales() {
                return totales;
            }

            /**
             * Sets the value of the totales property.
             * 
             * @param value
             *     allowed object is
             *     {@link TotalesFactExp }
             *     
             */
            public void setTotales(TotalesFactExp value) {
                this.totales = value;
            }

        }

    }


    /**
     * <p>Java class for anonymous complex type.
     * 
     * <p>The following schema fragment specifies the expected content contained within this class.
     * 
     * <pre>
     * &lt;complexType>
     *   &lt;complexContent>
     *     &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
     *       &lt;sequence>
     *         &lt;element name="TmstFirma" type="{http://cfe.dgi.gub.uy}FechaHoraType"/>
     *         &lt;element name="Encabezado">
     *           &lt;complexType>
     *             &lt;complexContent>
     *               &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
     *                 &lt;sequence>
     *                   &lt;element name="IdDoc" type="{http://cfe.dgi.gub.uy}IdDoc_Rem"/>
     *                   &lt;element name="Emisor" type="{http://cfe.dgi.gub.uy}Emisor"/>
     *                   &lt;element name="Receptor" type="{http://cfe.dgi.gub.uy}Receptor_Rem"/>
     *                   &lt;element name="Totales">
     *                     &lt;complexType>
     *                       &lt;complexContent>
     *                         &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
     *                           &lt;sequence>
     *                             &lt;element name="CantLinDet">
     *                               &lt;simpleType>
     *                                 &lt;restriction base="{http://cfe.dgi.gub.uy}LineasDetType">
     *                                   &lt;totalDigits value="3"/>
     *                                   &lt;fractionDigits value="0"/>
     *                                   &lt;maxInclusive value="200"/>
     *                                 &lt;/restriction>
     *                               &lt;/simpleType>
     *                             &lt;/element>
     *                           &lt;/sequence>
     *                         &lt;/restriction>
     *                       &lt;/complexContent>
     *                     &lt;/complexType>
     *                   &lt;/element>
     *                 &lt;/sequence>
     *               &lt;/restriction>
     *             &lt;/complexContent>
     *           &lt;/complexType>
     *         &lt;/element>
     *         &lt;element name="Detalle">
     *           &lt;complexType>
     *             &lt;complexContent>
     *               &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
     *                 &lt;sequence>
     *                   &lt;element name="Item" type="{http://cfe.dgi.gub.uy}Item_Rem" maxOccurs="200"/>
     *                 &lt;/sequence>
     *               &lt;/restriction>
     *             &lt;/complexContent>
     *           &lt;/complexType>
     *         &lt;/element>
     *         &lt;element name="SubTotInfo" type="{http://cfe.dgi.gub.uy}SubTotInfo" minOccurs="0"/>
     *         &lt;element name="Referencia" type="{http://cfe.dgi.gub.uy}ReferenciaTipo" minOccurs="0"/>
     *         &lt;element name="CAEData" type="{http://cfe.dgi.gub.uy}CAEDataType"/>
     *       &lt;/sequence>
     *     &lt;/restriction>
     *   &lt;/complexContent>
     * &lt;/complexType>
     * </pre>
     * 
     * 
     */
    @XmlAccessorType(XmlAccessType.FIELD)
    @XmlType(name = "", propOrder = {
        "tmstFirma",
        "encabezado",
        "detalle",
        "subTotInfo",
        "referencia",
        "caeData"
    })
    public static class ERem {

        @XmlElement(name = "TmstFirma", namespace = "http://cfe.dgi.gub.uy", required = true)
        protected XMLGregorianCalendar tmstFirma;
        @XmlElement(name = "Encabezado", namespace = "http://cfe.dgi.gub.uy", required = true)
        protected CFEDefType.ERem.Encabezado encabezado;
        @XmlElement(name = "Detalle", namespace = "http://cfe.dgi.gub.uy", required = true)
        protected CFEDefType.ERem.Detalle detalle;
        @XmlElement(name = "SubTotInfo", namespace = "http://cfe.dgi.gub.uy")
        protected SubTotInfo subTotInfo;
        @XmlElement(name = "Referencia", namespace = "http://cfe.dgi.gub.uy")
        protected ReferenciaTipo referencia;
        @XmlElement(name = "CAEData", namespace = "http://cfe.dgi.gub.uy", required = true)
        protected CAEDataType caeData;

        /**
         * Gets the value of the tmstFirma property.
         * 
         * @return
         *     possible object is
         *     {@link XMLGregorianCalendar }
         *     
         */
        public XMLGregorianCalendar getTmstFirma() {
            return tmstFirma;
        }

        /**
         * Sets the value of the tmstFirma property.
         * 
         * @param value
         *     allowed object is
         *     {@link XMLGregorianCalendar }
         *     
         */
        public void setTmstFirma(XMLGregorianCalendar value) {
            this.tmstFirma = value;
        }

        /**
         * Gets the value of the encabezado property.
         * 
         * @return
         *     possible object is
         *     {@link CFEDefType.ERem.Encabezado }
         *     
         */
        public CFEDefType.ERem.Encabezado getEncabezado() {
            return encabezado;
        }

        /**
         * Sets the value of the encabezado property.
         * 
         * @param value
         *     allowed object is
         *     {@link CFEDefType.ERem.Encabezado }
         *     
         */
        public void setEncabezado(CFEDefType.ERem.Encabezado value) {
            this.encabezado = value;
        }

        /**
         * Gets the value of the detalle property.
         * 
         * @return
         *     possible object is
         *     {@link CFEDefType.ERem.Detalle }
         *     
         */
        public CFEDefType.ERem.Detalle getDetalle() {
            return detalle;
        }

        /**
         * Sets the value of the detalle property.
         * 
         * @param value
         *     allowed object is
         *     {@link CFEDefType.ERem.Detalle }
         *     
         */
        public void setDetalle(CFEDefType.ERem.Detalle value) {
            this.detalle = value;
        }

        /**
         * Gets the value of the subTotInfo property.
         * 
         * @return
         *     possible object is
         *     {@link SubTotInfo }
         *     
         */
        public SubTotInfo getSubTotInfo() {
            return subTotInfo;
        }

        /**
         * Sets the value of the subTotInfo property.
         * 
         * @param value
         *     allowed object is
         *     {@link SubTotInfo }
         *     
         */
        public void setSubTotInfo(SubTotInfo value) {
            this.subTotInfo = value;
        }

        /**
         * Gets the value of the referencia property.
         * 
         * @return
         *     possible object is
         *     {@link ReferenciaTipo }
         *     
         */
        public ReferenciaTipo getReferencia() {
            return referencia;
        }

        /**
         * Sets the value of the referencia property.
         * 
         * @param value
         *     allowed object is
         *     {@link ReferenciaTipo }
         *     
         */
        public void setReferencia(ReferenciaTipo value) {
            this.referencia = value;
        }

        /**
         * Gets the value of the caeData property.
         * 
         * @return
         *     possible object is
         *     {@link CAEDataType }
         *     
         */
        public CAEDataType getCAEData() {
            return caeData;
        }

        /**
         * Sets the value of the caeData property.
         * 
         * @param value
         *     allowed object is
         *     {@link CAEDataType }
         *     
         */
        public void setCAEData(CAEDataType value) {
            this.caeData = value;
        }


        /**
         * <p>Java class for anonymous complex type.
         * 
         * <p>The following schema fragment specifies the expected content contained within this class.
         * 
         * <pre>
         * &lt;complexType>
         *   &lt;complexContent>
         *     &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
         *       &lt;sequence>
         *         &lt;element name="Item" type="{http://cfe.dgi.gub.uy}Item_Rem" maxOccurs="200"/>
         *       &lt;/sequence>
         *     &lt;/restriction>
         *   &lt;/complexContent>
         * &lt;/complexType>
         * </pre>
         * 
         * 
         */
        @XmlAccessorType(XmlAccessType.FIELD)
        @XmlType(name = "", propOrder = {
            "items"
        })
        public static class Detalle {

            @XmlElement(name = "Item", namespace = "http://cfe.dgi.gub.uy", required = true)
            protected List<ItemRem> items;

            /**
             * Gets the value of the items property.
             * 
             * <p>
             * This accessor method returns a reference to the live list,
             * not a snapshot. Therefore any modification you make to the
             * returned list will be present inside the JAXB object.
             * This is why there is not a <CODE>set</CODE> method for the items property.
             * 
             * <p>
             * For example, to add a new item, do as follows:
             * <pre>
             *    getItems().add(newItem);
             * </pre>
             * 
             * 
             * <p>
             * Objects of the following type(s) are allowed in the list
             * {@link ItemRem }
             * 
             * 
             */
            public List<ItemRem> getItems() {
                if (items == null) {
                    items = new ArrayList<ItemRem>();
                }
                return this.items;
            }

        }


        /**
         * <p>Java class for anonymous complex type.
         * 
         * <p>The following schema fragment specifies the expected content contained within this class.
         * 
         * <pre>
         * &lt;complexType>
         *   &lt;complexContent>
         *     &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
         *       &lt;sequence>
         *         &lt;element name="IdDoc" type="{http://cfe.dgi.gub.uy}IdDoc_Rem"/>
         *         &lt;element name="Emisor" type="{http://cfe.dgi.gub.uy}Emisor"/>
         *         &lt;element name="Receptor" type="{http://cfe.dgi.gub.uy}Receptor_Rem"/>
         *         &lt;element name="Totales">
         *           &lt;complexType>
         *             &lt;complexContent>
         *               &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
         *                 &lt;sequence>
         *                   &lt;element name="CantLinDet">
         *                     &lt;simpleType>
         *                       &lt;restriction base="{http://cfe.dgi.gub.uy}LineasDetType">
         *                         &lt;totalDigits value="3"/>
         *                         &lt;fractionDigits value="0"/>
         *                         &lt;maxInclusive value="200"/>
         *                       &lt;/restriction>
         *                     &lt;/simpleType>
         *                   &lt;/element>
         *                 &lt;/sequence>
         *               &lt;/restriction>
         *             &lt;/complexContent>
         *           &lt;/complexType>
         *         &lt;/element>
         *       &lt;/sequence>
         *     &lt;/restriction>
         *   &lt;/complexContent>
         * &lt;/complexType>
         * </pre>
         * 
         * 
         */
        @XmlAccessorType(XmlAccessType.FIELD)
        @XmlType(name = "", propOrder = {
            "idDoc",
            "emisor",
            "receptor",
            "totales"
        })
        public static class Encabezado {

            @XmlElement(name = "IdDoc", namespace = "http://cfe.dgi.gub.uy", required = true)
            protected IdDocRem idDoc;
            @XmlElement(name = "Emisor", namespace = "http://cfe.dgi.gub.uy", required = true)
            protected Emisor emisor;
            @XmlElement(name = "Receptor", namespace = "http://cfe.dgi.gub.uy", required = true)
            protected ReceptorRem receptor;
            @XmlElement(name = "Totales", namespace = "http://cfe.dgi.gub.uy", required = true)
            protected CFEDefType.ERem.Encabezado.Totales totales;

            /**
             * Gets the value of the idDoc property.
             * 
             * @return
             *     possible object is
             *     {@link IdDocRem }
             *     
             */
            public IdDocRem getIdDoc() {
                return idDoc;
            }

            /**
             * Sets the value of the idDoc property.
             * 
             * @param value
             *     allowed object is
             *     {@link IdDocRem }
             *     
             */
            public void setIdDoc(IdDocRem value) {
                this.idDoc = value;
            }

            /**
             * Gets the value of the emisor property.
             * 
             * @return
             *     possible object is
             *     {@link Emisor }
             *     
             */
            public Emisor getEmisor() {
                return emisor;
            }

            /**
             * Sets the value of the emisor property.
             * 
             * @param value
             *     allowed object is
             *     {@link Emisor }
             *     
             */
            public void setEmisor(Emisor value) {
                this.emisor = value;
            }

            /**
             * Gets the value of the receptor property.
             * 
             * @return
             *     possible object is
             *     {@link ReceptorRem }
             *     
             */
            public ReceptorRem getReceptor() {
                return receptor;
            }

            /**
             * Sets the value of the receptor property.
             * 
             * @param value
             *     allowed object is
             *     {@link ReceptorRem }
             *     
             */
            public void setReceptor(ReceptorRem value) {
                this.receptor = value;
            }

            /**
             * Gets the value of the totales property.
             * 
             * @return
             *     possible object is
             *     {@link CFEDefType.ERem.Encabezado.Totales }
             *     
             */
            public CFEDefType.ERem.Encabezado.Totales getTotales() {
                return totales;
            }

            /**
             * Sets the value of the totales property.
             * 
             * @param value
             *     allowed object is
             *     {@link CFEDefType.ERem.Encabezado.Totales }
             *     
             */
            public void setTotales(CFEDefType.ERem.Encabezado.Totales value) {
                this.totales = value;
            }


            /**
             * <p>Java class for anonymous complex type.
             * 
             * <p>The following schema fragment specifies the expected content contained within this class.
             * 
             * <pre>
             * &lt;complexType>
             *   &lt;complexContent>
             *     &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
             *       &lt;sequence>
             *         &lt;element name="CantLinDet">
             *           &lt;simpleType>
             *             &lt;restriction base="{http://cfe.dgi.gub.uy}LineasDetType">
             *               &lt;totalDigits value="3"/>
             *               &lt;fractionDigits value="0"/>
             *               &lt;maxInclusive value="200"/>
             *             &lt;/restriction>
             *           &lt;/simpleType>
             *         &lt;/element>
             *       &lt;/sequence>
             *     &lt;/restriction>
             *   &lt;/complexContent>
             * &lt;/complexType>
             * </pre>
             * 
             * 
             */
            @XmlAccessorType(XmlAccessType.FIELD)
            @XmlType(name = "", propOrder = {
                "cantLinDet"
            })
            public static class Totales {

                @XmlElement(name = "CantLinDet", namespace = "http://cfe.dgi.gub.uy")
                protected int cantLinDet;

                /**
                 * Gets the value of the cantLinDet property.
                 * 
                 */
                public int getCantLinDet() {
                    return cantLinDet;
                }

                /**
                 * Sets the value of the cantLinDet property.
                 * 
                 */
                public void setCantLinDet(int value) {
                    this.cantLinDet = value;
                }

            }

        }

    }


    /**
     * <p>Java class for anonymous complex type.
     * 
     * <p>The following schema fragment specifies the expected content contained within this class.
     * 
     * <pre>
     * &lt;complexType>
     *   &lt;complexContent>
     *     &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
     *       &lt;sequence>
     *         &lt;element name="TmstFirma" type="{http://cfe.dgi.gub.uy}FechaHoraType"/>
     *         &lt;element name="Encabezado">
     *           &lt;complexType>
     *             &lt;complexContent>
     *               &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
     *                 &lt;sequence>
     *                   &lt;element name="IdDoc" type="{http://cfe.dgi.gub.uy}IdDoc_Rem_Exp"/>
     *                   &lt;element name="Emisor" type="{http://cfe.dgi.gub.uy}Emisor"/>
     *                   &lt;element name="Receptor" type="{http://cfe.dgi.gub.uy}Receptor_Rem_Exp"/>
     *                   &lt;element name="Totales" type="{http://cfe.dgi.gub.uy}Totales_Rem_Exp"/>
     *                 &lt;/sequence>
     *               &lt;/restriction>
     *             &lt;/complexContent>
     *           &lt;/complexType>
     *         &lt;/element>
     *         &lt;element name="Detalle">
     *           &lt;complexType>
     *             &lt;complexContent>
     *               &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
     *                 &lt;sequence>
     *                   &lt;element name="Item" type="{http://cfe.dgi.gub.uy}Item_Rem_Exp" maxOccurs="200"/>
     *                 &lt;/sequence>
     *               &lt;/restriction>
     *             &lt;/complexContent>
     *           &lt;/complexType>
     *         &lt;/element>
     *         &lt;element name="SubTotInfo" type="{http://cfe.dgi.gub.uy}SubTotInfo" minOccurs="0"/>
     *         &lt;element name="Referencia" type="{http://cfe.dgi.gub.uy}ReferenciaTipo" minOccurs="0"/>
     *         &lt;element name="CAEData" type="{http://cfe.dgi.gub.uy}CAEDataType"/>
     *       &lt;/sequence>
     *     &lt;/restriction>
     *   &lt;/complexContent>
     * &lt;/complexType>
     * </pre>
     * 
     * 
     */
    @XmlAccessorType(XmlAccessType.FIELD)
    @XmlType(name = "", propOrder = {
        "tmstFirma",
        "encabezado",
        "detalle",
        "subTotInfo",
        "referencia",
        "caeData"
    })
    public static class ERemExp {

        @XmlElement(name = "TmstFirma", namespace = "http://cfe.dgi.gub.uy", required = true)
        protected XMLGregorianCalendar tmstFirma;
        @XmlElement(name = "Encabezado", namespace = "http://cfe.dgi.gub.uy", required = true)
        protected CFEDefType.ERemExp.Encabezado encabezado;
        @XmlElement(name = "Detalle", namespace = "http://cfe.dgi.gub.uy", required = true)
        protected CFEDefType.ERemExp.Detalle detalle;
        @XmlElement(name = "SubTotInfo", namespace = "http://cfe.dgi.gub.uy")
        protected SubTotInfo subTotInfo;
        @XmlElement(name = "Referencia", namespace = "http://cfe.dgi.gub.uy")
        protected ReferenciaTipo referencia;
        @XmlElement(name = "CAEData", namespace = "http://cfe.dgi.gub.uy", required = true)
        protected CAEDataType caeData;

        /**
         * Gets the value of the tmstFirma property.
         * 
         * @return
         *     possible object is
         *     {@link XMLGregorianCalendar }
         *     
         */
        public XMLGregorianCalendar getTmstFirma() {
            return tmstFirma;
        }

        /**
         * Sets the value of the tmstFirma property.
         * 
         * @param value
         *     allowed object is
         *     {@link XMLGregorianCalendar }
         *     
         */
        public void setTmstFirma(XMLGregorianCalendar value) {
            this.tmstFirma = value;
        }

        /**
         * Gets the value of the encabezado property.
         * 
         * @return
         *     possible object is
         *     {@link CFEDefType.ERemExp.Encabezado }
         *     
         */
        public CFEDefType.ERemExp.Encabezado getEncabezado() {
            return encabezado;
        }

        /**
         * Sets the value of the encabezado property.
         * 
         * @param value
         *     allowed object is
         *     {@link CFEDefType.ERemExp.Encabezado }
         *     
         */
        public void setEncabezado(CFEDefType.ERemExp.Encabezado value) {
            this.encabezado = value;
        }

        /**
         * Gets the value of the detalle property.
         * 
         * @return
         *     possible object is
         *     {@link CFEDefType.ERemExp.Detalle }
         *     
         */
        public CFEDefType.ERemExp.Detalle getDetalle() {
            return detalle;
        }

        /**
         * Sets the value of the detalle property.
         * 
         * @param value
         *     allowed object is
         *     {@link CFEDefType.ERemExp.Detalle }
         *     
         */
        public void setDetalle(CFEDefType.ERemExp.Detalle value) {
            this.detalle = value;
        }

        /**
         * Gets the value of the subTotInfo property.
         * 
         * @return
         *     possible object is
         *     {@link SubTotInfo }
         *     
         */
        public SubTotInfo getSubTotInfo() {
            return subTotInfo;
        }

        /**
         * Sets the value of the subTotInfo property.
         * 
         * @param value
         *     allowed object is
         *     {@link SubTotInfo }
         *     
         */
        public void setSubTotInfo(SubTotInfo value) {
            this.subTotInfo = value;
        }

        /**
         * Gets the value of the referencia property.
         * 
         * @return
         *     possible object is
         *     {@link ReferenciaTipo }
         *     
         */
        public ReferenciaTipo getReferencia() {
            return referencia;
        }

        /**
         * Sets the value of the referencia property.
         * 
         * @param value
         *     allowed object is
         *     {@link ReferenciaTipo }
         *     
         */
        public void setReferencia(ReferenciaTipo value) {
            this.referencia = value;
        }

        /**
         * Gets the value of the caeData property.
         * 
         * @return
         *     possible object is
         *     {@link CAEDataType }
         *     
         */
        public CAEDataType getCAEData() {
            return caeData;
        }

        /**
         * Sets the value of the caeData property.
         * 
         * @param value
         *     allowed object is
         *     {@link CAEDataType }
         *     
         */
        public void setCAEData(CAEDataType value) {
            this.caeData = value;
        }


        /**
         * <p>Java class for anonymous complex type.
         * 
         * <p>The following schema fragment specifies the expected content contained within this class.
         * 
         * <pre>
         * &lt;complexType>
         *   &lt;complexContent>
         *     &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
         *       &lt;sequence>
         *         &lt;element name="Item" type="{http://cfe.dgi.gub.uy}Item_Rem_Exp" maxOccurs="200"/>
         *       &lt;/sequence>
         *     &lt;/restriction>
         *   &lt;/complexContent>
         * &lt;/complexType>
         * </pre>
         * 
         * 
         */
        @XmlAccessorType(XmlAccessType.FIELD)
        @XmlType(name = "", propOrder = {
            "items"
        })
        public static class Detalle {

            @XmlElement(name = "Item", namespace = "http://cfe.dgi.gub.uy", required = true)
            protected List<ItemRemExp> items;

            /**
             * Gets the value of the items property.
             * 
             * <p>
             * This accessor method returns a reference to the live list,
             * not a snapshot. Therefore any modification you make to the
             * returned list will be present inside the JAXB object.
             * This is why there is not a <CODE>set</CODE> method for the items property.
             * 
             * <p>
             * For example, to add a new item, do as follows:
             * <pre>
             *    getItems().add(newItem);
             * </pre>
             * 
             * 
             * <p>
             * Objects of the following type(s) are allowed in the list
             * {@link ItemRemExp }
             * 
             * 
             */
            public List<ItemRemExp> getItems() {
                if (items == null) {
                    items = new ArrayList<ItemRemExp>();
                }
                return this.items;
            }

        }


        /**
         * <p>Java class for anonymous complex type.
         * 
         * <p>The following schema fragment specifies the expected content contained within this class.
         * 
         * <pre>
         * &lt;complexType>
         *   &lt;complexContent>
         *     &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
         *       &lt;sequence>
         *         &lt;element name="IdDoc" type="{http://cfe.dgi.gub.uy}IdDoc_Rem_Exp"/>
         *         &lt;element name="Emisor" type="{http://cfe.dgi.gub.uy}Emisor"/>
         *         &lt;element name="Receptor" type="{http://cfe.dgi.gub.uy}Receptor_Rem_Exp"/>
         *         &lt;element name="Totales" type="{http://cfe.dgi.gub.uy}Totales_Rem_Exp"/>
         *       &lt;/sequence>
         *     &lt;/restriction>
         *   &lt;/complexContent>
         * &lt;/complexType>
         * </pre>
         * 
         * 
         */
        @XmlAccessorType(XmlAccessType.FIELD)
        @XmlType(name = "", propOrder = {
            "idDoc",
            "emisor",
            "receptor",
            "totales"
        })
        public static class Encabezado {

            @XmlElement(name = "IdDoc", namespace = "http://cfe.dgi.gub.uy", required = true)
            protected IdDocRemExp idDoc;
            @XmlElement(name = "Emisor", namespace = "http://cfe.dgi.gub.uy", required = true)
            protected Emisor emisor;
            @XmlElement(name = "Receptor", namespace = "http://cfe.dgi.gub.uy", required = true)
            protected ReceptorRemExp receptor;
            @XmlElement(name = "Totales", namespace = "http://cfe.dgi.gub.uy", required = true)
            protected TotalesRemExp totales;

            /**
             * Gets the value of the idDoc property.
             * 
             * @return
             *     possible object is
             *     {@link IdDocRemExp }
             *     
             */
            public IdDocRemExp getIdDoc() {
                return idDoc;
            }

            /**
             * Sets the value of the idDoc property.
             * 
             * @param value
             *     allowed object is
             *     {@link IdDocRemExp }
             *     
             */
            public void setIdDoc(IdDocRemExp value) {
                this.idDoc = value;
            }

            /**
             * Gets the value of the emisor property.
             * 
             * @return
             *     possible object is
             *     {@link Emisor }
             *     
             */
            public Emisor getEmisor() {
                return emisor;
            }

            /**
             * Sets the value of the emisor property.
             * 
             * @param value
             *     allowed object is
             *     {@link Emisor }
             *     
             */
            public void setEmisor(Emisor value) {
                this.emisor = value;
            }

            /**
             * Gets the value of the receptor property.
             * 
             * @return
             *     possible object is
             *     {@link ReceptorRemExp }
             *     
             */
            public ReceptorRemExp getReceptor() {
                return receptor;
            }

            /**
             * Sets the value of the receptor property.
             * 
             * @param value
             *     allowed object is
             *     {@link ReceptorRemExp }
             *     
             */
            public void setReceptor(ReceptorRemExp value) {
                this.receptor = value;
            }

            /**
             * Gets the value of the totales property.
             * 
             * @return
             *     possible object is
             *     {@link TotalesRemExp }
             *     
             */
            public TotalesRemExp getTotales() {
                return totales;
            }

            /**
             * Sets the value of the totales property.
             * 
             * @param value
             *     allowed object is
             *     {@link TotalesRemExp }
             *     
             */
            public void setTotales(TotalesRemExp value) {
                this.totales = value;
            }

        }

    }


    /**
     * <p>Java class for anonymous complex type.
     * 
     * <p>The following schema fragment specifies the expected content contained within this class.
     * 
     * <pre>
     * &lt;complexType>
     *   &lt;complexContent>
     *     &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
     *       &lt;sequence>
     *         &lt;element name="TmstFirma" type="{http://cfe.dgi.gub.uy}FechaHoraType"/>
     *         &lt;element name="Encabezado">
     *           &lt;complexType>
     *             &lt;complexContent>
     *               &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
     *                 &lt;sequence>
     *                   &lt;element name="IdDoc" type="{http://cfe.dgi.gub.uy}IdDoc_Resg"/>
     *                   &lt;element name="Emisor" type="{http://cfe.dgi.gub.uy}Emisor"/>
     *                   &lt;element name="Receptor" type="{http://cfe.dgi.gub.uy}Receptor_Resg"/>
     *                   &lt;element name="Totales" type="{http://cfe.dgi.gub.uy}Totales_Resg"/>
     *                 &lt;/sequence>
     *               &lt;/restriction>
     *             &lt;/complexContent>
     *           &lt;/complexType>
     *         &lt;/element>
     *         &lt;element name="Detalle">
     *           &lt;complexType>
     *             &lt;complexContent>
     *               &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
     *                 &lt;sequence>
     *                   &lt;element name="Item" type="{http://cfe.dgi.gub.uy}Item_Resg" maxOccurs="200"/>
     *                 &lt;/sequence>
     *               &lt;/restriction>
     *             &lt;/complexContent>
     *           &lt;/complexType>
     *         &lt;/element>
     *         &lt;element name="SubTotInfo" type="{http://cfe.dgi.gub.uy}SubTotInfo" minOccurs="0"/>
     *         &lt;element name="Referencia" type="{http://cfe.dgi.gub.uy}ReferenciaTipo" minOccurs="0"/>
     *         &lt;element name="CAEData" type="{http://cfe.dgi.gub.uy}CAEDataType"/>
     *       &lt;/sequence>
     *     &lt;/restriction>
     *   &lt;/complexContent>
     * &lt;/complexType>
     * </pre>
     * 
     * 
     */
    @XmlAccessorType(XmlAccessType.FIELD)
    @XmlType(name = "", propOrder = {
        "tmstFirma",
        "encabezado",
        "detalle",
        "subTotInfo",
        "referencia",
        "caeData"
    })
    public static class EResg {

        @XmlElement(name = "TmstFirma", namespace = "http://cfe.dgi.gub.uy", required = true)
        protected XMLGregorianCalendar tmstFirma;
        @XmlElement(name = "Encabezado", namespace = "http://cfe.dgi.gub.uy", required = true)
        protected CFEDefType.EResg.Encabezado encabezado;
        @XmlElement(name = "Detalle", namespace = "http://cfe.dgi.gub.uy", required = true)
        protected CFEDefType.EResg.Detalle detalle;
        @XmlElement(name = "SubTotInfo", namespace = "http://cfe.dgi.gub.uy")
        protected SubTotInfo subTotInfo;
        @XmlElement(name = "Referencia", namespace = "http://cfe.dgi.gub.uy")
        protected ReferenciaTipo referencia;
        @XmlElement(name = "CAEData", namespace = "http://cfe.dgi.gub.uy", required = true)
        protected CAEDataType caeData;

        /**
         * Gets the value of the tmstFirma property.
         * 
         * @return
         *     possible object is
         *     {@link XMLGregorianCalendar }
         *     
         */
        public XMLGregorianCalendar getTmstFirma() {
            return tmstFirma;
        }

        /**
         * Sets the value of the tmstFirma property.
         * 
         * @param value
         *     allowed object is
         *     {@link XMLGregorianCalendar }
         *     
         */
        public void setTmstFirma(XMLGregorianCalendar value) {
            this.tmstFirma = value;
        }

        /**
         * Gets the value of the encabezado property.
         * 
         * @return
         *     possible object is
         *     {@link CFEDefType.EResg.Encabezado }
         *     
         */
        public CFEDefType.EResg.Encabezado getEncabezado() {
            return encabezado;
        }

        /**
         * Sets the value of the encabezado property.
         * 
         * @param value
         *     allowed object is
         *     {@link CFEDefType.EResg.Encabezado }
         *     
         */
        public void setEncabezado(CFEDefType.EResg.Encabezado value) {
            this.encabezado = value;
        }

        /**
         * Gets the value of the detalle property.
         * 
         * @return
         *     possible object is
         *     {@link CFEDefType.EResg.Detalle }
         *     
         */
        public CFEDefType.EResg.Detalle getDetalle() {
            return detalle;
        }

        /**
         * Sets the value of the detalle property.
         * 
         * @param value
         *     allowed object is
         *     {@link CFEDefType.EResg.Detalle }
         *     
         */
        public void setDetalle(CFEDefType.EResg.Detalle value) {
            this.detalle = value;
        }

        /**
         * Gets the value of the subTotInfo property.
         * 
         * @return
         *     possible object is
         *     {@link SubTotInfo }
         *     
         */
        public SubTotInfo getSubTotInfo() {
            return subTotInfo;
        }

        /**
         * Sets the value of the subTotInfo property.
         * 
         * @param value
         *     allowed object is
         *     {@link SubTotInfo }
         *     
         */
        public void setSubTotInfo(SubTotInfo value) {
            this.subTotInfo = value;
        }

        /**
         * Gets the value of the referencia property.
         * 
         * @return
         *     possible object is
         *     {@link ReferenciaTipo }
         *     
         */
        public ReferenciaTipo getReferencia() {
            return referencia;
        }

        /**
         * Sets the value of the referencia property.
         * 
         * @param value
         *     allowed object is
         *     {@link ReferenciaTipo }
         *     
         */
        public void setReferencia(ReferenciaTipo value) {
            this.referencia = value;
        }

        /**
         * Gets the value of the caeData property.
         * 
         * @return
         *     possible object is
         *     {@link CAEDataType }
         *     
         */
        public CAEDataType getCAEData() {
            return caeData;
        }

        /**
         * Sets the value of the caeData property.
         * 
         * @param value
         *     allowed object is
         *     {@link CAEDataType }
         *     
         */
        public void setCAEData(CAEDataType value) {
            this.caeData = value;
        }


        /**
         * <p>Java class for anonymous complex type.
         * 
         * <p>The following schema fragment specifies the expected content contained within this class.
         * 
         * <pre>
         * &lt;complexType>
         *   &lt;complexContent>
         *     &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
         *       &lt;sequence>
         *         &lt;element name="Item" type="{http://cfe.dgi.gub.uy}Item_Resg" maxOccurs="200"/>
         *       &lt;/sequence>
         *     &lt;/restriction>
         *   &lt;/complexContent>
         * &lt;/complexType>
         * </pre>
         * 
         * 
         */
        @XmlAccessorType(XmlAccessType.FIELD)
        @XmlType(name = "", propOrder = {
            "items"
        })
        public static class Detalle {

            @XmlElement(name = "Item", namespace = "http://cfe.dgi.gub.uy", required = true)
            protected List<ItemResg> items;

            /**
             * Gets the value of the items property.
             * 
             * <p>
             * This accessor method returns a reference to the live list,
             * not a snapshot. Therefore any modification you make to the
             * returned list will be present inside the JAXB object.
             * This is why there is not a <CODE>set</CODE> method for the items property.
             * 
             * <p>
             * For example, to add a new item, do as follows:
             * <pre>
             *    getItems().add(newItem);
             * </pre>
             * 
             * 
             * <p>
             * Objects of the following type(s) are allowed in the list
             * {@link ItemResg }
             * 
             * 
             */
            public List<ItemResg> getItems() {
                if (items == null) {
                    items = new ArrayList<ItemResg>();
                }
                return this.items;
            }

        }


        /**
         * <p>Java class for anonymous complex type.
         * 
         * <p>The following schema fragment specifies the expected content contained within this class.
         * 
         * <pre>
         * &lt;complexType>
         *   &lt;complexContent>
         *     &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
         *       &lt;sequence>
         *         &lt;element name="IdDoc" type="{http://cfe.dgi.gub.uy}IdDoc_Resg"/>
         *         &lt;element name="Emisor" type="{http://cfe.dgi.gub.uy}Emisor"/>
         *         &lt;element name="Receptor" type="{http://cfe.dgi.gub.uy}Receptor_Resg"/>
         *         &lt;element name="Totales" type="{http://cfe.dgi.gub.uy}Totales_Resg"/>
         *       &lt;/sequence>
         *     &lt;/restriction>
         *   &lt;/complexContent>
         * &lt;/complexType>
         * </pre>
         * 
         * 
         */
        @XmlAccessorType(XmlAccessType.FIELD)
        @XmlType(name = "", propOrder = {
            "idDoc",
            "emisor",
            "receptor",
            "totales"
        })
        public static class Encabezado {

            @XmlElement(name = "IdDoc", namespace = "http://cfe.dgi.gub.uy", required = true)
            protected IdDocResg idDoc;
            @XmlElement(name = "Emisor", namespace = "http://cfe.dgi.gub.uy", required = true)
            protected Emisor emisor;
            @XmlElement(name = "Receptor", namespace = "http://cfe.dgi.gub.uy", required = true)
            protected ReceptorResg receptor;
            @XmlElement(name = "Totales", namespace = "http://cfe.dgi.gub.uy", required = true)
            protected TotalesResg totales;

            /**
             * Gets the value of the idDoc property.
             * 
             * @return
             *     possible object is
             *     {@link IdDocResg }
             *     
             */
            public IdDocResg getIdDoc() {
                return idDoc;
            }

            /**
             * Sets the value of the idDoc property.
             * 
             * @param value
             *     allowed object is
             *     {@link IdDocResg }
             *     
             */
            public void setIdDoc(IdDocResg value) {
                this.idDoc = value;
            }

            /**
             * Gets the value of the emisor property.
             * 
             * @return
             *     possible object is
             *     {@link Emisor }
             *     
             */
            public Emisor getEmisor() {
                return emisor;
            }

            /**
             * Sets the value of the emisor property.
             * 
             * @param value
             *     allowed object is
             *     {@link Emisor }
             *     
             */
            public void setEmisor(Emisor value) {
                this.emisor = value;
            }

            /**
             * Gets the value of the receptor property.
             * 
             * @return
             *     possible object is
             *     {@link ReceptorResg }
             *     
             */
            public ReceptorResg getReceptor() {
                return receptor;
            }

            /**
             * Sets the value of the receptor property.
             * 
             * @param value
             *     allowed object is
             *     {@link ReceptorResg }
             *     
             */
            public void setReceptor(ReceptorResg value) {
                this.receptor = value;
            }

            /**
             * Gets the value of the totales property.
             * 
             * @return
             *     possible object is
             *     {@link TotalesResg }
             *     
             */
            public TotalesResg getTotales() {
                return totales;
            }

            /**
             * Sets the value of the totales property.
             * 
             * @param value
             *     allowed object is
             *     {@link TotalesResg }
             *     
             */
            public void setTotales(TotalesResg value) {
                this.totales = value;
            }

        }

    }


    /**
     * <p>Java class for anonymous complex type.
     * 
     * <p>The following schema fragment specifies the expected content contained within this class.
     * 
     * <pre>
     * &lt;complexType>
     *   &lt;complexContent>
     *     &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
     *       &lt;sequence>
     *         &lt;element name="TmstFirma" type="{http://cfe.dgi.gub.uy}FechaHoraType"/>
     *         &lt;element name="Encabezado">
     *           &lt;complexType>
     *             &lt;complexContent>
     *               &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
     *                 &lt;sequence>
     *                   &lt;element name="IdDoc" type="{http://cfe.dgi.gub.uy}IdDoc_Tck"/>
     *                   &lt;element name="Emisor" type="{http://cfe.dgi.gub.uy}Emisor"/>
     *                   &lt;element name="Receptor" type="{http://cfe.dgi.gub.uy}Receptor_Tck" minOccurs="0"/>
     *                   &lt;element name="Totales" type="{http://cfe.dgi.gub.uy}Totales"/>
     *                 &lt;/sequence>
     *               &lt;/restriction>
     *             &lt;/complexContent>
     *           &lt;/complexType>
     *         &lt;/element>
     *         &lt;element name="Detalle">
     *           &lt;complexType>
     *             &lt;complexContent>
     *               &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
     *                 &lt;sequence>
     *                   &lt;element name="Item" type="{http://cfe.dgi.gub.uy}Item_Det_Fact" maxOccurs="700"/>
     *                 &lt;/sequence>
     *               &lt;/restriction>
     *             &lt;/complexContent>
     *           &lt;/complexType>
     *         &lt;/element>
     *         &lt;element name="SubTotInfo" type="{http://cfe.dgi.gub.uy}SubTotInfo" minOccurs="0"/>
     *         &lt;element name="DscRcgGlobal" type="{http://cfe.dgi.gub.uy}DscRcgGlobal" minOccurs="0"/>
     *         &lt;element name="MediosPago" type="{http://cfe.dgi.gub.uy}MediosPago" minOccurs="0"/>
     *         &lt;element name="Referencia" type="{http://cfe.dgi.gub.uy}ReferenciaTipo" minOccurs="0"/>
     *         &lt;element name="CAEData" type="{http://cfe.dgi.gub.uy}CAEDataType"/>
     *         &lt;element name="Compl_Fiscal" type="{http://cfe.dgi.gub.uy}Compl_FiscalType" minOccurs="0"/>
     *       &lt;/sequence>
     *     &lt;/restriction>
     *   &lt;/complexContent>
     * &lt;/complexType>
     * </pre>
     * 
     * 
     */
    @XmlAccessorType(XmlAccessType.FIELD)
    @XmlType(name = "", propOrder = {
        "tmstFirma",
        "encabezado",
        "detalle",
        "subTotInfo",
        "dscRcgGlobal",
        "mediosPago",
        "referencia",
        "caeData",
        "complFiscal"
    })
    public static class ETck {

        @XmlElement(name = "TmstFirma", namespace = "http://cfe.dgi.gub.uy", required = true)
        protected XMLGregorianCalendar tmstFirma;
        @XmlElement(name = "Encabezado", namespace = "http://cfe.dgi.gub.uy", required = true)
        protected CFEDefType.ETck.Encabezado encabezado;
        @XmlElement(name = "Detalle", namespace = "http://cfe.dgi.gub.uy", required = true)
        protected CFEDefType.ETck.Detalle detalle;
        @XmlElement(name = "SubTotInfo", namespace = "http://cfe.dgi.gub.uy")
        protected SubTotInfo subTotInfo;
        @XmlElement(name = "DscRcgGlobal", namespace = "http://cfe.dgi.gub.uy")
        protected DscRcgGlobal dscRcgGlobal;
        @XmlElement(name = "MediosPago", namespace = "http://cfe.dgi.gub.uy")
        protected MediosPago mediosPago;
        @XmlElement(name = "Referencia", namespace = "http://cfe.dgi.gub.uy")
        protected ReferenciaTipo referencia;
        @XmlElement(name = "CAEData", namespace = "http://cfe.dgi.gub.uy", required = true)
        protected CAEDataType caeData;
        @XmlElement(name = "Compl_Fiscal", namespace = "http://cfe.dgi.gub.uy")
        protected ComplFiscalType complFiscal;

        /**
         * Gets the value of the tmstFirma property.
         * 
         * @return
         *     possible object is
         *     {@link XMLGregorianCalendar }
         *     
         */
        public XMLGregorianCalendar getTmstFirma() {
            return tmstFirma;
        }

        /**
         * Sets the value of the tmstFirma property.
         * 
         * @param value
         *     allowed object is
         *     {@link XMLGregorianCalendar }
         *     
         */
        public void setTmstFirma(XMLGregorianCalendar value) {
            this.tmstFirma = value;
        }

        /**
         * Gets the value of the encabezado property.
         * 
         * @return
         *     possible object is
         *     {@link CFEDefType.ETck.Encabezado }
         *     
         */
        public CFEDefType.ETck.Encabezado getEncabezado() {
            return encabezado;
        }

        /**
         * Sets the value of the encabezado property.
         * 
         * @param value
         *     allowed object is
         *     {@link CFEDefType.ETck.Encabezado }
         *     
         */
        public void setEncabezado(CFEDefType.ETck.Encabezado value) {
            this.encabezado = value;
        }

        /**
         * Gets the value of the detalle property.
         * 
         * @return
         *     possible object is
         *     {@link CFEDefType.ETck.Detalle }
         *     
         */
        public CFEDefType.ETck.Detalle getDetalle() {
            return detalle;
        }

        /**
         * Sets the value of the detalle property.
         * 
         * @param value
         *     allowed object is
         *     {@link CFEDefType.ETck.Detalle }
         *     
         */
        public void setDetalle(CFEDefType.ETck.Detalle value) {
            this.detalle = value;
        }

        /**
         * Gets the value of the subTotInfo property.
         * 
         * @return
         *     possible object is
         *     {@link SubTotInfo }
         *     
         */
        public SubTotInfo getSubTotInfo() {
            return subTotInfo;
        }

        /**
         * Sets the value of the subTotInfo property.
         * 
         * @param value
         *     allowed object is
         *     {@link SubTotInfo }
         *     
         */
        public void setSubTotInfo(SubTotInfo value) {
            this.subTotInfo = value;
        }

        /**
         * Gets the value of the dscRcgGlobal property.
         * 
         * @return
         *     possible object is
         *     {@link DscRcgGlobal }
         *     
         */
        public DscRcgGlobal getDscRcgGlobal() {
            return dscRcgGlobal;
        }

        /**
         * Sets the value of the dscRcgGlobal property.
         * 
         * @param value
         *     allowed object is
         *     {@link DscRcgGlobal }
         *     
         */
        public void setDscRcgGlobal(DscRcgGlobal value) {
            this.dscRcgGlobal = value;
        }

        /**
         * Gets the value of the mediosPago property.
         * 
         * @return
         *     possible object is
         *     {@link MediosPago }
         *     
         */
        public MediosPago getMediosPago() {
            return mediosPago;
        }

        /**
         * Sets the value of the mediosPago property.
         * 
         * @param value
         *     allowed object is
         *     {@link MediosPago }
         *     
         */
        public void setMediosPago(MediosPago value) {
            this.mediosPago = value;
        }

        /**
         * Gets the value of the referencia property.
         * 
         * @return
         *     possible object is
         *     {@link ReferenciaTipo }
         *     
         */
        public ReferenciaTipo getReferencia() {
            return referencia;
        }

        /**
         * Sets the value of the referencia property.
         * 
         * @param value
         *     allowed object is
         *     {@link ReferenciaTipo }
         *     
         */
        public void setReferencia(ReferenciaTipo value) {
            this.referencia = value;
        }

        /**
         * Gets the value of the caeData property.
         * 
         * @return
         *     possible object is
         *     {@link CAEDataType }
         *     
         */
        public CAEDataType getCAEData() {
            return caeData;
        }

        /**
         * Sets the value of the caeData property.
         * 
         * @param value
         *     allowed object is
         *     {@link CAEDataType }
         *     
         */
        public void setCAEData(CAEDataType value) {
            this.caeData = value;
        }

        /**
         * Gets the value of the complFiscal property.
         * 
         * @return
         *     possible object is
         *     {@link ComplFiscalType }
         *     
         */
        public ComplFiscalType getComplFiscal() {
            return complFiscal;
        }

        /**
         * Sets the value of the complFiscal property.
         * 
         * @param value
         *     allowed object is
         *     {@link ComplFiscalType }
         *     
         */
        public void setComplFiscal(ComplFiscalType value) {
            this.complFiscal = value;
        }


        /**
         * <p>Java class for anonymous complex type.
         * 
         * <p>The following schema fragment specifies the expected content contained within this class.
         * 
         * <pre>
         * &lt;complexType>
         *   &lt;complexContent>
         *     &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
         *       &lt;sequence>
         *         &lt;element name="Item" type="{http://cfe.dgi.gub.uy}Item_Det_Fact" maxOccurs="700"/>
         *       &lt;/sequence>
         *     &lt;/restriction>
         *   &lt;/complexContent>
         * &lt;/complexType>
         * </pre>
         * 
         * 
         */
        @XmlAccessorType(XmlAccessType.FIELD)
        @XmlType(name = "", propOrder = {
            "items"
        })
        public static class Detalle {

            @XmlElement(name = "Item", namespace = "http://cfe.dgi.gub.uy", required = true)
            protected List<ItemDetFact> items;

            /**
             * Gets the value of the items property.
             * 
             * <p>
             * This accessor method returns a reference to the live list,
             * not a snapshot. Therefore any modification you make to the
             * returned list will be present inside the JAXB object.
             * This is why there is not a <CODE>set</CODE> method for the items property.
             * 
             * <p>
             * For example, to add a new item, do as follows:
             * <pre>
             *    getItems().add(newItem);
             * </pre>
             * 
             * 
             * <p>
             * Objects of the following type(s) are allowed in the list
             * {@link ItemDetFact }
             * 
             * 
             */
            public List<ItemDetFact> getItems() {
                if (items == null) {
                    items = new ArrayList<ItemDetFact>();
                }
                return this.items;
            }

        }


        /**
         * <p>Java class for anonymous complex type.
         * 
         * <p>The following schema fragment specifies the expected content contained within this class.
         * 
         * <pre>
         * &lt;complexType>
         *   &lt;complexContent>
         *     &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
         *       &lt;sequence>
         *         &lt;element name="IdDoc" type="{http://cfe.dgi.gub.uy}IdDoc_Tck"/>
         *         &lt;element name="Emisor" type="{http://cfe.dgi.gub.uy}Emisor"/>
         *         &lt;element name="Receptor" type="{http://cfe.dgi.gub.uy}Receptor_Tck" minOccurs="0"/>
         *         &lt;element name="Totales" type="{http://cfe.dgi.gub.uy}Totales"/>
         *       &lt;/sequence>
         *     &lt;/restriction>
         *   &lt;/complexContent>
         * &lt;/complexType>
         * </pre>
         * 
         * 
         */
        @XmlAccessorType(XmlAccessType.FIELD)
        @XmlType(name = "", propOrder = {
            "idDoc",
            "emisor",
            "receptor",
            "totales"
        })
        public static class Encabezado {

            @XmlElement(name = "IdDoc", namespace = "http://cfe.dgi.gub.uy", required = true)
            protected IdDocTck idDoc;
            @XmlElement(name = "Emisor", namespace = "http://cfe.dgi.gub.uy", required = true)
            protected Emisor emisor;
            @XmlElement(name = "Receptor", namespace = "http://cfe.dgi.gub.uy")
            protected ReceptorTck receptor;
            @XmlElement(name = "Totales", namespace = "http://cfe.dgi.gub.uy", required = true)
            protected dgi.classes.recepcion.Totales totales;

            /**
             * Gets the value of the idDoc property.
             * 
             * @return
             *     possible object is
             *     {@link IdDocTck }
             *     
             */
            public IdDocTck getIdDoc() {
                return idDoc;
            }

            /**
             * Sets the value of the idDoc property.
             * 
             * @param value
             *     allowed object is
             *     {@link IdDocTck }
             *     
             */
            public void setIdDoc(IdDocTck value) {
                this.idDoc = value;
            }

            /**
             * Gets the value of the emisor property.
             * 
             * @return
             *     possible object is
             *     {@link Emisor }
             *     
             */
            public Emisor getEmisor() {
                return emisor;
            }

            /**
             * Sets the value of the emisor property.
             * 
             * @param value
             *     allowed object is
             *     {@link Emisor }
             *     
             */
            public void setEmisor(Emisor value) {
                this.emisor = value;
            }

            /**
             * Gets the value of the receptor property.
             * 
             * @return
             *     possible object is
             *     {@link ReceptorTck }
             *     
             */
            public ReceptorTck getReceptor() {
                return receptor;
            }

            /**
             * Sets the value of the receptor property.
             * 
             * @param value
             *     allowed object is
             *     {@link ReceptorTck }
             *     
             */
            public void setReceptor(ReceptorTck value) {
                this.receptor = value;
            }

            /**
             * Gets the value of the totales property.
             * 
             * @return
             *     possible object is
             *     {@link dgi.classes.recepcion.Totales }
             *     
             */
            public dgi.classes.recepcion.Totales getTotales() {
                return totales;
            }

            /**
             * Sets the value of the totales property.
             * 
             * @param value
             *     allowed object is
             *     {@link dgi.classes.recepcion.Totales }
             *     
             */
            public void setTotales(dgi.classes.recepcion.Totales value) {
                this.totales = value;
            }

        }

    }

}
